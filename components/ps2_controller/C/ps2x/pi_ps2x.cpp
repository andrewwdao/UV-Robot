...
void PS2X::update(void)
{
    ...
    if ((this->ps2data[1] == 0x41)||
        ...
        // store as one variable. We only monitor the stick changes to trigger the readAnalog
        // so a normal plus is enough, no need to shift register.
        // You can add another one for Right one if you like. in my application scope I don't need it, so I didn't create it.
        this->Lsticks = this->ps2data[7] + this->ps2data[8];
